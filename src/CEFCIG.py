...
def gridgo():
    ...
    if args is not None:
        ...
        results = [train_df, parameter_df]
        save_obj(results, output_dir+'GridGo.pkl')


def predictgo():
    if (len(sys.argv) < 8) and ('-h' not in sys.argv) and ('--help' not in sys.argv):
        # at least one parameter need to be specified, will print help message if no parameter is specified
        print "\nusage:\n\nCEFCIG PredictGo <gene meta table> <gene GTF> <target wig meta table> <gene expression><output_dir>" \
              "\n\nfor more help, please try: CEFCIG PredictGo -h\n"
        return 1

    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter,
                                     usage="\n\nCEFCIG PredictGo <gene meta table> <gridgo_obj> <gene GTF> <target wig meta table> <gene expression><output_dir>>\n\n",
                                     description='', epilog="Chen lab, Houston Methodist")
    ...
    parser.add_argument('output_dir', default=None,
                        help="output directory")
    parser.add_argument('-t', '--test_predict', dest="test_predict", metavar='', default=None, \
                        help="feature table for test only")
    ...
    if args is not None:
        if args.test_predict is None:
            ...
            training_table, parameters = gridgo_obj[0], gridgo_obj[1]
        ...
        # save_obj(predict_obj, output_dir + '/predictGo.pkl')
            ...
            test_predict_path = args.test_predict
            if test_predict_path.endswith('.xlsx'):
                test_predict = pd.read_excel(test_predict_path, index_col=0)
            elif test_predict_path.endswith('.csv'):
                test_predict = pd.read_csv(test_predict_path, index_col=0)
            elif test_predict_path.endswith('.xls') or test_predict_path.endswith('.txt'):
                test_predict = pd.read_csv(test_predict_path, sep='\t', index_col=0)
            else:
                test_predict = None
            ...
            for gene_id in gtf.index:
                ...
                if chr == 'chrM':
                    continue
                gene_obj = Gene(gene_id, 'test', None, chr, start, end)
                gene_objs.append(gene_obj)
            ...
            predict_obj = PredictGo(gene_objs, gtf, training_table, parameters, preknown, gene_meta_df, test_predict)

            predict_obj.predict()

            final_df = predict_obj.prediction
            final_df.to_csv(output_dir + 'prediction_test_results.csv')
